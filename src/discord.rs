    ...
    pub fn update(&mut self, is_connected: bool, session_info: &SessionInfo, delta: f32) {
        ...
        if let Some(client) = &mut self.client {
            let details = if is_connected {
                if session_info.car_name == "-" { "In Pit / Idle".to_string() }
                else { format!("Driving {}", session_info.car_name) }
            } else {
                "In Menu".to_string()
            };

            let state = if is_connected {
                format!("{} | Lap {}", session_info.track_name, session_info.lap_count)
            } else {
                "Analyzing Telemetry".to_string()
            };

            // Маленький текст при наведении
