...
fn render_central_gauges(f: &mut Frame<'_>, area: Rect, app: &AppState) {
    let theme = &app.ui_state.theme;
    let lang = &app.config.language;
    let block = Block::default()
    ...
    if let Some(phys) = &app.physics_mem {
        ...
        let speed_gear = Paragraph::new(vec![Line::from(vec![
            Span::styled(
                format!("{:3}", data.speed_kmh as i32),
                Style::default()
    ...
    let delta_widget = Paragraph::new(vec![Line::from(vec![
        Span::styled(
            format!("{}: ", tr("lbl_delta", lang)),
            Style::default().fg(app.ui_state.get_color(&theme.text)),
        ),
        Span::styled(
    ...
    if let Some(gfx) = &app.graphics_mem {
        ...
        let electronics = Paragraph::new(vec![
            ...
            Line::from(vec![
                Span::styled(
                    format!("{}: ", tr("lbl_map", lang)),
                    Style::default().fg(app.ui_state.get_color(&theme.text)),
                ),
                Span::styled(
                    format!("{}", data.engine_map),
...
fn render_info_panel(f: &mut Frame<'_>, area: Rect, app: &AppState) {
    ...
    let info_lines = vec![
        ...
        Line::from(vec![
            Span::styled(
                format!("{}: ", tr("lbl_session", lang)),
                Style::default().fg(app.ui_state.get_color(&theme.text)),
            ),
            Span::styled(
                &app.session_info.session_type,
        ...
        Line::from(vec![
            Span::styled(
                format!("{}: ", tr("lbl_laps", lang)),
                Style::default().fg(app.ui_state.get_color(&theme.text)),
            ),
            Span::styled(
                format!("{}", app.session_info.lap_count),
                Style::default().fg(Color::Cyan),
            ),
        ]),
        Line::from(vec![
            Span::styled(
                format!("{}: ", tr("lbl_pos", lang)),
                Style::default().fg(app.ui_state.get_color(&theme.text)),
            ),
            Span::styled(
                if let Some(gfx) = &app.graphics_mem {
            ...
            Span::styled(
                if let Some(phys) = &app.physics_mem {
                    format!(
                        "{:.1}L ({:.1} {})",
                        phys.get().fuel,
                        app.engineer.stats.fuel_laps_remaining,
                        if app.config.language == crate::config::Language::Russian {
                            "кр"
                        } else {
                            "laps"
                        }
    ...
    let info_widget = Paragraph::new(info_lines).block(Block::default());
    ...
    let info = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                app.graphics_mem
                    .as_ref()
                    .map_or("-".to_string(), |g| g.get().tc.to_string()),
                Style::default().fg(Color::Yellow),
            ),
            Span::raw("  ABS: "),
            Span::styled(
                app.graphics_mem
                    .as_ref()
                    .map_or("-".to_string(), |g| g.get().abs.to_string()),
                Style::default().fg(Color::Cyan),
            ),
        ]),
    ];
