...
pub fn render_vertical(f: &mut Frame, area: Rect, app: &AppState) {
    let text = Paragraph::new("Vertical Engineer View")
        .block(Block::default().title(tr("tab_eng", &app.config.language)).borders(Borders::ALL))
        .alignment(Alignment::Center);
    f.render_widget(text, area);
}

fn render_recommendations(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    if app.recommendations.is_empty() {
        ...
        let items: Vec<ListItem> = app.recommendations.iter().take(8).map(|rec| {
            ...
            let mut lines = vec![
                ...
                Line::from(vec![
                    Span::raw("   "),
                    Span::styled(format!("{}: ", tr("eng_action", lang)), Style::default().fg(Color::DarkGray)),
...
fn render_analysis(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    let analysis = vec![
        Line::from(vec![
            Span::styled(format!("{}: ", tr("eng_smooth", lang)), Style::default().fg(app.ui_state.get_color(&theme.text))),
            render_progress_bar(app.engineer.driving_style.smoothness, 100.0),
        ]),
        Line::from(vec![
            Span::styled(format!("{}: ", tr("eng_aggr", lang)), Style::default().fg(app.ui_state.get_color(&theme.text))),
            render_progress_bar(app.engineer.driving_style.aggression * 100.0, 100.0),
        ]),
        Line::from(vec![
            Span::styled(format!("{}: ", tr("eng_trail", lang)), Style::default().fg(app.ui_state.get_color(&theme.text))),
            render_progress_bar(app.engineer.driving_style.trail_braking * 100.0, 100.0),
        ]),
        Line::from(""),
        Line::from(vec![
            Span::styled(format!("{}: ", tr("eng_lock", lang)), Style::default().fg(app.ui_state.get_color(&theme.text))),
            Span::styled(
        ...
        Line::from(vec![
            Span::styled(format!("{}: ", tr("eng_spin", lang)), Style::default().fg(app.ui_state.get_color(&theme.text))),
            Span::styled(
