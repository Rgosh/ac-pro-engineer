...
pub fn render(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    let layout = Layout::default()
        ...
        .split(overlay_area.inner(&Margin { vertical: 1, horizontal: 1 }));
    ...
    if let Some(phys) = &app.physics_mem {
        ...
        let gear_char = match p.gear {
            0 => "R", 1 => "N", n => return render_gear_num(f, layout[0], n - 1, p.rpms, app),
        };
        render_gear_text(f, layout[0], gear_char, Color::Red);

        let speed_text = Paragraph::new(vec![
            Line::from(Span::styled(format!("{:.0}", p.speed_kmh), Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD))),
        ...
        let delta_text = Paragraph::new(vec![
            Line::from(Span::styled(format!("{:+.3}", delta), Style::default().fg(d_color).add_modifier(Modifier::BOLD))),
        ...
        let fuel_text = Paragraph::new(vec![
            Line::from(Span::styled(format!("{:.1} L", p.fuel), Style::default().fg(get_fuel_color(app.engineer.stats.fuel_laps_remaining)))),
            Line::from(Span::styled(format!("~{:.1} laps", app.engineer.stats.fuel_laps_remaining), Style::default().fg(Color::White))),
...
fn render_gear_num(f: &mut Frame, area: Rect, gear: i32, rpm: i32, app: &AppState) {
    
    let max_rpm = app.session_info.max_rpm;
    let color = if rpm > max_rpm - 200 { Color::Red } else { Color::Yellow };
...
fn render_gear_text(f: &mut Frame, area: Rect, text: &str, color: Color) {
