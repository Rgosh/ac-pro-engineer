...
local function drawOverlayPanel(x, y)
  ...
  -- Title bar, so the thing on screen is recognisably a CSP app window.
  love.graphics.setColor(0.13, 0.14, 0.17, 1)
  love.graphics.rectangle('fill', x, y - 22, w, 22, 4, 4)
  love.graphics.setColor(0.62, 0.66, 0.72, 1)
  love.graphics.setFont(harness.titleFont)
  love.graphics.print('AC Pro Engineer', x + 8, y - 19)

  drawBackdrop(x, y, w, h)

  if not app.loaded then
    love.graphics.setColor(1, 0.34, 0.34, 1)
    love.graphics.printf(app.error or 'app not loaded', x + 8, y + 12, w - 16)
    return
  end
...
function love.draw()
  ...
  local controlX = panelX + S.panelWidth + 24
